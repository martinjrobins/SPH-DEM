...
inline void calcEnstrophyAndAveDens(Cparticle &p,CglobalVars &g) {
   g.custom[0] += 0.5*p.mass*pow(p.vort[0],2)/p.dens;
...
inline void calcpressgrad(Cparticle &p,vector<Cparticle *> &neighbrs,CglobalVars &g) {
   p.fp = 0;
   int n = neighbrs.size();
   for (int i=0;i<n;i++) {
      Cparticle *pn = neighbrs[i];
      ...
      const vect gradWa = gradW(p,*pn,dx,q,hav);
      p.fp += pn->mass*(p.press-pn->press)*gradWa;
   }
}


void CcustomOutput::calcOutput(int outstep,CcustomSim *custSim,Cio_data_vtk *io) {
   data->neighboursGroup<calcpressgrad,ifSph>();
/*
