...
const double INLET_RADIUS = 3.0/1000.0;
const double INLET_HEIGHT = 10.0/1000.0;
const double INLET_FLOW_RATE = 100.0;
const double INFLOW_VEL = INLET_FLOW_RATE*1e-6/(PI*pow(INLET_RADIUS,2)*60.0);
const double RMIN[NDIM] = {CYLINDER_ORIGIN[0]-CYLINDER_RADIUS-0.1,CYLINDER_ORIGIN[1]-CYLINDER_RADIUS-0.1,CYLINDER_ORIGIN[2]-INLET_HEIGHT-0.1};
const double RMAX[NDIM] = {CYLINDER_ORIGIN[0]+CYLINDER_RADIUS+0.1,CYLINDER_ORIGIN[1]+CYLINDER_RADIUS+0.1,CYLINDER_ORIGIN[2]+CYLINDER_HEIGHT+0.1};
...
const int NX = 15;
const int NY = NX;
const double PSEP = 0.5*(BMAX[0]-BMIN[0])/(NX+1.6);
...
const double DEM_RADIUS = 2.0*0.167/1000.0;
const double DEM_DENS = DENS*2.5;
...
const double DEM_K = 1.0e6;
const double DEM_GAMMA = 0.5;
...
#ifdef LIQ_DEM
const double NDEM = SOLIDVOL/DEM_VOL;
#else
const double NDEM = 0;
#endif
...
const int MAX_NUM_PARTICLES_PER_CPU = 1.5*NX*NY*CYLINDER_HEIGHT/PSEP + 1.0*NDEM + 2.0*DEM_DISK_NDEM;
const int MAX_NUM_DEM_PARTICLES = 1.0*NDEM + 2.0*DEM_DISK_NDEM;
