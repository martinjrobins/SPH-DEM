...
inline void printVel(Cparticle &p,CglobalVars &g) {
   CcustomOutput::fo << g.time <<' '<<p.vhat[0]<<' '<<p.vhat[1]<<' '<<p.vhat[2]<<' '<<p.fp[0]<<' '<<p.fp[1]<<' '<<p.fp[2]<<' '<<p.fv[0]<<' '<<p.fv[1]<<' '<<p.fv[2]<<' '<<p.fb[0]<<' '<<p.fb[1]<<' '<<p.fb[2]<<' '<<p.ff[0]<<' '<<p.ff[1]<<' '<<p.ff[2]<<' '<<p.porosity<<' '<<p.r[0]<<' '<<p.r[1]<<' '<<p.r[2]<<' '<<p.fvel[0]<<' '<<p.fvel[1]<<' '<<p.fvel[2]<<endl;
}
