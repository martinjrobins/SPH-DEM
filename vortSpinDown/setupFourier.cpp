...
int main() {
   ...
   Array<double,NDIM> FourierCoeff(FOURIER_N+1,FOURIER_N+1);
   Array<double,NDIM> XFourierCoeff(FOURIER_N+1,FOURIER_N+1);
   Array<double,NDIM> YFourierCoeff(FOURIER_N+1,FOURIER_N+1);

   gsl_rng *rng = gsl_rng_alloc(gsl_rng_ranlxd1);
   gsl_rng_set(rng,FOURIER_SEED);

   const double iMult = 2*PI/(RMAX[0]-RMIN[0]);
   const double jMult = 2*PI/(RMAX[1]-RMIN[1]);

   for (int i=0;i<=FOURIER_N;i++) {
      for (int j=0;j<=FOURIER_N;j++) {
         double sigma;
         if ((i==FOURIER_N)||(j==FOURIER_N)) {
            sigma = 0.0;
         } else {
            sigma = (i/double(1+pow(i/8.0,4)))*(j/double(1+pow(j/8.0,4)));
         }
         FourierCoeff(i,j) = gsl_ran_gaussian(rng,sigma);
	 XFourierCoeff(i,j) = jMult*j*FourierCoeff(i,j);
	 YFourierCoeff(i,j) = -iMult*i*FourierCoeff(i,j);
      }
   }
   gsl_rng_free(rng);
      ...
      for (int j=-2;j<=NY+2;j++) {
         ...
         */
         if ((j<=0)||(j>=NY)||(i<=0)||(i>=NX)) {
            ...
            vect vRand = 0.0;
            for (int a=0;a<=FOURIER_N;a++) {
               for (int b=0;b<=FOURIER_N;b++) {
                  vRand[0] += XFourierCoeff(a,b)*cos(a*acos(p.r[0]))*cos(b*acos(p.r[1]));
                  vRand[1] += YFourierCoeff(a,b)*cos(a*acos(p.r[0]))*cos(b*acos(p.r[1]));
               }
            }
